...
public class LogsSaver {
    ...
    public void log(String msg) {
        try( PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(LOGS_DIR + "\\" + NAME_PREFIX + df.format(new Date()) + NAME_SUFFIX, true))) ) {
            ...
            log.log(Level.WARNING, java.util.ResourceBundle.getBundle("com/hulist/bundle/LogsSaver").getString("BŁĄD PODCZAS ZAPISU DANYCH DO LOGU."));
    ...
    private void cleanupDir() {
        ...
        for( Map.Entry<Date, File> entry : dates.entrySet() ) {
            if( counter < filesToDelete ){
                ...
                if( !file.delete() ){
                    log.log(Level.WARNING, String.format(java.util.ResourceBundle.getBundle("com/hulist/bundle/LogsSaver").getString("BŁĄD PODCZAS USUWANIA LOGU %S"), file.getName()));
                }
                counter++;
            } else {
                break;
